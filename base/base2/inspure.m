...
global glv
    ...
    for k=1:nn:len-nn+1    % 每累计n个imu数据更新一次姿态，即n子样算法
        k1 = k+nn-1;
        wvm = imu(k:k1, 1:6);  t = imu(k1,end);    % 提出该时段内的imu中角增量和速度增量序列，有nn个值以及后续的时间戳
        ins = insupdate(ins, wvm);    % 更新一次ins结构体，avp数据存于结构体中，即更新一次姿态，位置，速度，输入为上一时刻的avp信息以及该新输入的imu提供的校准角增量以及速度增量信息 
        %% 
        ins.eth.dgnt=t; % ins.vn(3) = 0;
